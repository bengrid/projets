...
import ordi from "../../images/ordinateur.png";

const UserProduit = (props) => {
  const userProduit = props.data;
  const [playonce, Setplayonce] = useState(true);
  const [produit2, Setproduit2] = useState([]);
  useEffect(() => {
    if (playonce) {
      axios
        .get(`http://localhost:3001/api/produit/${userProduit}`)
        .then((res) => {
          Setproduit2(res.data);
          Setplayonce(false);
        });
    }
  });
  ...
  return (
    <>
      <div className="border-2 p-4  md:flex md:flex-row justify-start mb-8 space-x-6 border-gray-300 shadow-sm rounded">
        <Link
          to={`${URL_DETAIL_PRODUIT}/${userProduit}`}
          className="md:w-1/4 mb-4"
        >
          <img className="w-full" src={ordi} alt="img" />
        </Link>
        <div className="md:w-3/4 lg:w-full">
          <Link to={`${URL_DETAIL_PRODUIT}/${userProduit}`} className="">
            <h1 className="uppercase text-black font-semibold">
              {produit2.nom}
            </h1>
            <h1 className="uppercase text-black font-semibold">
              Réference: {produit2.reference}
            </h1>
            <p className="space-x-2 text-justify">{produit2.details}</p>
            <p>
              <strong>{produit2.price} €</strong>
            </p>
          </Link>
